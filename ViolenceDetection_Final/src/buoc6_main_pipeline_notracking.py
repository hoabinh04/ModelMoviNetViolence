...
FIGHT_ON_THRESHOLD = 0.65
FIGHT_OFF_THRESHOLD = 0.45
ADAPTIVE_ON_MAX_REDUCTION = 0.10

MIN_VIOLENCE_STREAK = 3
MIN_NORMAL_STREAK = 12
...
PROFILE_PRESETS = {
    ...
    "far_camera": {
        "MIN_ACTIVATION_SCORE": 270,
        "PAIR_MIN_NET_MARGIN": 55.0,
        "SCENE_MARGIN_REQUIRED": 45.0,
        "MIN_INTERACTION_FRAMES": 5,
        "MIN_PROXIMITY_FRAMES": 5,
        "APPROACH_SPEED_MIN": 1.0,
        "PROXIMITY_REL_SPEED_MIN": 2.0,
        "PROXIMITY_APPROACH_MIN": 2.2,
        "REACQUIRE_SCORE_MIN": 260,
        "SINGLE_LOCK_MIN_SPEED": 1.4,
        "HUMAN_MIN_HEIGHT_RATIO": 0.015,
        "HUMAN_MIN_AREA_RATIO": 0.0002,
        "LOW_CONF_BOX_THRESHOLD": 0.15,
        "LOW_CONF_MIN_HEIGHT_RATIO": 0.025,
        "ALT_CLASS_MIN_CONF": 0.12,
        "ALT_CLASS_MIN_HEIGHT_RATIO": 0.025,
        "WIDE_BOX_MIN_HEIGHT_RATIO": 0.05,
        "WIDE_BOX_MIN_AREA_RATIO": 0.007,
        "MERGED_BOX_MIN_AREA_RATIO": 0.010,
        "MIN_TUBE_SIZE": 120,
        "ROI_EXPAND_RATIO": 2.4,
        "SEARCH_EXPAND_MULT": 1.35,
        "GATE_V_THRESHOLD": 8.5,
        "GATE_A_THRESHOLD": 4.5,
        "FULLFRAME_RESCUE_MIN_SINGLE_GATE_STREAK": 0,
        "FULLFRAME_RESCUE_BLEND": 0.95,
    },
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tsm-variant", choices=["small", "large"], default="large")
    parser.add_argument("--yolo-weights", type=Path, default=DEFAULT_YOLO_WEIGHTS)
    parser.add_argument("--video-path", type=str, default=str(DEFAULT_TEST_VIDEO))
    ...
    parser.add_argument("--det-conf", type=float, default=DEFAULT_DET_CONF)
...
def resolve_video_source(raw_path: str) -> Tuple[object, Optional[Path]]:
    raw_path = str(raw_path).strip()
    if raw_path.isdigit():
        return int(raw_path), None
...
def main() -> None:
    ...
    if args.profile == "far_camera" and abs(args.det_conf - DEFAULT_DET_CONF) < 1e-6:
        args.det_conf = 0.12
    if args.profile == "far_camera" and abs(args.tsm_only_on_threshold - 0.50) < 1e-6:
        args.tsm_only_on_threshold = 0.42
    if args.profile == "far_camera" and abs(args.tsm_only_off_threshold - 0.36) < 1e-6:
        args.tsm_only_off_threshold = 0.30
    ...
    video_source, video_path = resolve_video_source(args.video_path)
    ...
    print(
        ...
        f"tsm_variant={args.tsm_variant} | decision={args.decision_mode} | "
        f"fullframe_rescue={fullframe_rescue_enabled}"
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    yolo_model = YOLO(str(args.yolo_weights))
    tsm_model = MobileNetV3_TSM(
        num_classes=2,
        num_segments=args.num_segments,
        pretrained=False,
        variant=args.tsm_variant,
    )
    ...
    if not cap.isOpened():
        raise RuntimeError(f"Cannot open video: {video_path or video_source}")
    ...
    while True:
        ...
        if len(boxes) > 0:
            ...
            if active_tube_center is not None and tube_life > 0:
                if frame_idx % 10 == 0:
                    ...
                    cv2.imwrite(os.path.join(debug_dir, f"roi_{frame_idx:04d}.jpg"), raw_frame)
                    
                full_img = Image.fromarray(cv2.cvtColor(raw_frame, cv2.COLOR_BGR2RGB))
                
                # Bypassing the crop entirely. Putting full image into both buffers
                frame_buffer.append(transform(full_img))
                ...
                if (
                    ...
                    # Giảm bớt sức ảnh hưởng của Điểm hiền lành để tránh ép phần trăm về 0 quá mạnh
                    benign_penalty = clamp01(current_benign_score / 500.0) * dynamic_benign_scale * 0.5

                    fused_prob = clamp01(
                        0.85 * stable_raw_prob
                        + 0.10 * kinematic_prob
                        + 0.05 * scene_prob
                        + evidence_bonus
                        - benign_penalty
                    )
                    # Phục hồi độ mượt cũ để tránh giật lag phần trăm (flickering)
                    ...
                    if args.decision_mode == "tsm_only":
                        if args.tsm_only_use_raw_boost:
                            tsm_decision_prob = max(smooth_prob, stable_raw_prob, fused_prob, raw_fight_prob)
                        else:
                        ...
                        if alarm_on and (
                            ...
                            or ((smooth_prob < 0.15 and current_active_score < 150) and selective_cooldown == 0)
                        ):
                            alarm_on = False
        ...
        display_on_threshold = args.tsm_only_on_threshold if args.decision_mode == "tsm_only" else on_threshold_now
        ...
        cv2.putText(
            vis_frame,
                f"raw={raw_fight_prob:.2f} fused={fused_prob:.2f} smooth={smooth_prob:.2f} on={display_on_threshold:.2f} margin={display_scene_margin:.0f} act={dynamic_min_activation:.0f} det={len(track_data):02d}",
            (10, 56),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.58,
            (255, 255, 255),
